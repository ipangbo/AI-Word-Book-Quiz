...
import { themes, applyTheme, ThemeName, ThemeMode, SEED_COLORS, FontSizeLevel, applyFontSize, FONT_SIZE_KEY } from '../utils/theme';
import { Palette, Info, Mic, Volume2, Gauge, Play, Activity, Check, ChevronDown, Keyboard, FlaskConical, FileCode, Loader2, Download, Search, Clock, Trash2, History, ListChecks, Sun, Moon, Monitor, Type } from 'lucide-react';
import { getEnglishVoices, getTTSConfig, saveTTSConfig, TTSConfig, speak } from '../utils/tts';
import { getDictationSettings, saveDictationSettings, getMCSettings, saveMCSettings } from '../utils/settings';
import { DictationGlobalSettings, MultipleChoiceGlobalSettings, WordEntry } from '../types';
...
// Permanent files that will always show up in dev tools
...
export const SettingsScreen: React.FC<SettingsScreenProps> = ({ onQuickImport, showToast }) => {
  ...
  const [recentFiles, setRecentFiles] = useState<string[]>(() => {
    ...
    // Ensure permanent files are always present at the start, followed by unique history items
  ...
  return (
    ...
    >
      ...
      <div className="bg-white dark:bg-md-surface-container rounded-3xl p-6 border border-md-surface-container shadow-sm mb-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              <div>
                  <div className="flex justify-between mb-2">
                      ...
                      <span className="text-base font-mono text-md-primary">{(dictSettings.correctDelay / 1000).toFixed(1)}s</span>
                  </div>
                  <input 
              ...
              <div>
                  <div className="flex justify-between mb-2">
                      ...
                      <span className="text-base font-mono text-md-primary">{(dictSettings.incorrectDelay / 1000).toFixed(1)}s</span>
                  </div>
                  <input 
       ...
       {/* --- Multiple Choice Settings Section --- */}
       <div className="bg-white dark:bg-md-surface-container rounded-3xl p-6 border border-md-surface-container shadow-sm mb-6">
          <div className="flex items-center gap-3 mb-6">
             ...
             <div className="flex gap-2">
               {[2, 3, 4, 6].map(opt => (
                 <ChoiceChip
                   key={opt}
                   label={opt.toString()}
                   selected={mcSettings.defaultOptionCount === opt}
                   onClick={() => handleMCSettingChange('defaultOptionCount', opt)}
                 />
               ))}
             </div>
          </div>
          ...
          <div className="space-y-1">
              <p className="text-sm font-bold text-md-outline uppercase mb-2 tracking-wider">Visible Hints</p>
              <Switch 
                checked={mcSettings.defaultShowPhonetic} 
                onChange={(v) => handleMCSettingChange('defaultShowPhonetic', v)}
                label="Show Phonetic"
              />
              <Switch 
                checked={mcSettings.defaultShowPos} 
                onChange={(v) => handleMCSettingChange('defaultShowPos', v)}
                label="Show Part of Speech"
              />
              <Switch 
                checked={mcSettings.defaultShowDefinition} 
                onChange={(v) => handleMCSettingChange('defaultShowDefinition', v)}
                label="Show Definition"
              />
              <Switch 
                checked={mcSettings.defaultShowTranslation} 
                onChange={(v) => handleMCSettingChange('defaultShowTranslation', v)}
                label="Show Translation (CN)"
              />
          </div>
      </div>
        ...
        <div className="mb-6 relative" ref={dropdownRef}>
          ...
          <AnimatePresence>
            {isVoiceDropdownOpen && (
              ...
              >
                <div 
                   className={`p-4 cursor-pointer flex items-center justify-between hover:bg-md-on-surface/5 transition-colors relative ${!ttsConfig.voiceURI ? 'bg-md-secondary-container/50 dark:bg-md-secondary-container/30' : ''}`}
                   onClick={() => {
                ...
                >
                   <Ripple />
                   <span className={`text-base ${!ttsConfig.voiceURI ? 'text-md-primary font-bold' : 'text-md-on-surface'}`}>Default System Voice</span>
                   {!ttsConfig.voiceURI && <Check size={18} className="text-md-primary" />}
                </div>
                {voices.map(v => (
                  <div
                    key={v.voiceURI}
                    className={`p-4 cursor-pointer flex items-center justify-between hover:bg-md-on-surface/5 transition-colors relative ${ttsConfig.voiceURI === v.voiceURI ? 'bg-md-secondary-container/50 dark:bg-md-secondary-container/30' : ''}`}
                    onClick={() => {
                  ...
                  >
                    <Ripple />
                    <span className={`text-base truncate ${ttsConfig.voiceURI === v.voiceURI ? 'text-md-primary font-bold' : 'text-md-on-surface'}`}>{v.name}</span>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6 relative z-10">
            <div>
                <div className="flex justify-between mb-2">
                    ...
                    <span className="text-base font-mono text-md-primary">{ttsConfig.rate.toFixed(1)}x</span>
                </div>
                <input 
            ...
            <div>
                <div className="flex justify-between mb-2">
                    ...
                    <span className="text-base font-mono text-md-primary">{ttsConfig.pitch.toFixed(1)}</span>
                </div>
                <input 
                    type="range" min="0.5" max="2.0" step="0.1"
