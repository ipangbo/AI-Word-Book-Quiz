...
interface DictationSessionProps {
  ...
  onFinish: (mistakes: DictationMistake[]) => void;
  onExit: () => void;
}
...
export const DictationSession: React.FC<DictationSessionProps> = ({ entries, config, onFinish, onExit }) => {
  ...
  useEffect(() => {
    if (currentWord && feedback === 'idle') {
      ...
      // Delay focus slightly to account for AnimatePresence exit animation (mode="wait")
      // This ensures the input is actually in the DOM before we call .focus()
  ...
  const handleSubmit = (e?: React.FormEvent) => {
    ...
    if (feedback !== 'idle') return; // Prevent double submit
  ...
  const triggerFeedback = (status: FeedbackState, mistake?: DictationMistake) => {
    ...
    if (status === 'correct') speak("Correct");
    else speak("Incorrect. The answer is " + currentWord.word);
  ...
  // Helper to mask word in sentence
