...
export const DictationSession: React.FC<DictationSessionProps> = ({ entries, config, onFinish, onExit }) => {
  ...
  useEffect(() => {
    // Scroll inner container to top when index changes
    const container = document.getElementById('dictation-scroll-container');
          ...
          <AnimatePresence mode="wait">
            {feedback === 'idle' ? (
                ...
                >
                    ...
                    className="mb-6 relative group rounded-full"
                    ...
                    <span className="absolute -bottom-6 left-1/2 -translate-x-1/2 text-[10px] text-md-outline whitespace-nowrap opacity-60">Alt + R</span>
                    </button>

                    <div className="w-full space-y-3 mb-6 text-center min-h-[100px]">
                        <AnimatePresence>
                ...
                <motion.div 
                    ...
                    className="w-full max-w-md bg-white dark:bg-md-surface-container rounded-3xl p-6 shadow-xl border border-md-surface-container flex flex-col items-center text-center cursor-pointer relative overflow-hidden"
                ...
                >
                    ...
                    <div className={`w-16 h-16 rounded-full flex items-center justify-center mb-3 ${feedback === 'correct' ? 'bg-green-100 text-green-600' : 'bg-red-100 text-red-600'}`}>
                        {feedback === 'correct' ? <CheckCircle2 size={32} /> : <AlertCircle size={32} />}
                    </div>
                    <h2 className={`text-xl font-bold mb-2 ${feedback === 'correct' ? 'text-green-700' : 'text-red-700'}`}>
                        {feedback === 'correct' ? 'Correct!' : 'Incorrect'}
                    </h2>
                    <div className="my-2 w-full">
                        <p className="text-xs text-md-outline uppercase font-bold mb-1">Answer</p>
                        <p className="text-2xl font-bold text-md-on-surface mb-3">{currentWord.word}</p>
                        {feedback === 'incorrect' && (
                            <div className="bg-red-50 dark:bg-red-900/20 p-3 rounded-xl border border-red-100 dark:border-red-900/30">
                                <p className="text-[10px] text-red-500 font-bold uppercase mb-1">You Typed</p>
                    ...
                    <p className="text-[10px] text-md-outline mt-2 opacity-60">
      ...
      <div className="px-4 py-3 flex items-center gap-3 shrink-0 bg-md-surface/90 backdrop-blur-sm z-10">
        {feedback === 'idle' ? (
            <>
                <button
                onClick={handleSkip}
                className="relative overflow-hidden flex-1 py-3.5 rounded-2xl flex items-center justify-center gap-2 font-medium text-md-outline bg-md-surface-container hover:bg-md-secondary-container transition-colors"
                >
                <Ripple />
